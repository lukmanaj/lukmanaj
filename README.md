...
* 🧠  I'm currently learning `Foundations of Data Structures and Algorithms Specialization` on `Coursera` and I am also currently enrolled in Arewa Data Science Academy's `Deep Learning with PyTorch fellowship`, at the same time the `Natural Language Processing Specialization offered by DeepLearning.ai` on `Coursera`.
