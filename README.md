...
* 🧠  I'm currently learning `University of Colorado Boulder`'s `Foundations of Data Structures and Algorithms Specialization` and `DeepLearning.ai`'s `Natural Language Processing Specialization` on  `Coursera` and I am also currently enrolled in Arewa Data Science Academy's `Deep Learning with PyTorch fellowship`. 
